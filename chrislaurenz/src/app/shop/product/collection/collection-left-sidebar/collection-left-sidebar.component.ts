...
import { Product, ColorFilter, TagFilter, ProductCategory } from '../../../../shared/classes/product';
  ...
  constructor(private route: ActivatedRoute, private router: Router,
    private productsService: ProductsService) { 
       this.route.params.subscribe(params => {
          ...
          this.productsService.getProductByCategory(category).subscribe(
            data => { 
              this.allItems = data; 
          ...
          this.productsService.getProductCategories().subscribe(
            data => { 
              ...
              this.currentCategory = data.find((itemCategory: ProductCategory) => {
                let categoryEn = `en:${category}`, categoryDe = `de:${category}`, categoryFr = `fr:${category}`;
                return itemCategory.name.toLowerCase().includes(categoryEn.toLowerCase()) 
                || itemCategory.name.toLowerCase().includes(categoryDe.toLowerCase())
                || itemCategory.name.toLowerCase().includes(categoryFr.toLowerCase()); 
              });
            }, 
            error => { console.log(error); }
          );
  ...
  public filterItems(): Product[] {
      return this.items.filter((item: Product) => {
          const Colors: boolean = this.colorFilters.reduce((prev, curr) => { // Match Color
            if(item.variants.map(pv => pv.color)){
              if (item.variants.map(pv => pv.color).includes(curr.color)) {
                return prev && true;
              } 
            }
          }, true);
